                ...
                {footerData.map(({ name, path, icon: Icon }, index) => {
                    ...
                    return (
                        <Link to={path} key={index} className="flex flex-col items-center">
                            <Icon
                                className={`${isActive || profileActive ? "text-mainColor" : "text-placeholderColor"}`}
                            />
                            <p className={`font-medium text-[12px] ${isActive || profileActive ? "text-mainColor" : "text-placeholderColor"}`}>
                                {name}
                            </p>
                        </Link>
                    )
